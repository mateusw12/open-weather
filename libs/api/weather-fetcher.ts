...
export class WeatherFetcher {
  ...
  static async getGeo<T>(path: string, query: Record<string, string | number>) {
    const apiKey = process.env.NEXT_PUBLIC_WEATHER_API_KEY;
    ...
    const response = await fetch(`${GEO_API_BASE_URL}/${path}?${params.toString()}`, {
      next: { revalidate: 3600 },
    });

    if (!response.ok) {
      const details = await response.text();
      throw new Error(`OpenWeather geo request failed (${response.status}): ${details}`);
    }
